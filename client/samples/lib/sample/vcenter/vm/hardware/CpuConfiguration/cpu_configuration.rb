...
EOL
  ...
  def execute
    puts "Getting the VM Details #{vm_name}"
    @vm_id = VMHelper.get_vm(service_manager.vapi_config, @vm_name)
    @vm_cpu_info = @vm_svc.get(vm_id)
    puts "Number of CPU cores #{@vm_cpu_info.count}"
    puts "Number if cores per socket #{@vm_cpu_info.cores_per_socket}"
    puts "Hot add enabled #{@vm_cpu_info.hot_add_enabled}"
    puts "Hot remove enabled #{@vm_cpu_info.hot_remove_enabled}"
    ...
    @vm_cpu_info = @vm_svc.get(vm_id)
    puts "*** Updating the CPU-related settings of virtual machine #{@vm_name} ***" 
