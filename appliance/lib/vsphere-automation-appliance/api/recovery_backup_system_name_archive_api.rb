...
module VSphereAutomation
  ...
  class RecoveryBackupSystemNameArchiveApi
    ...
    # @return [ApplianceRecoveryBackupSystemNameArchiveResp|VapiStdErrorsErrorError|VapiStdErrorsNotFoundError|]
    def get(system_name, archive, spec_location, opts = {})
      data, _status_code, _headers = get_with_http_info(system_name, archive, spec_location, opts)
      data
    end
    ...
    # @option opts [String] :spec_location_user Username for the given location.
    # @option opts [String] :spec_location_password Password for the given location.
    # @return [Array<(ApplianceRecoveryBackupSystemNameArchiveResp|VapiStdErrorsErrorError|VapiStdErrorsNotFoundError|, Fixnum, Hash)>]  data, response status code and response headers
    def get_with_http_info(system_name, archive, spec_location, opts = {})
      ...
      # resource path
      local_var_path = '/rest/appliance/recovery/backup/system-name/{system_name}/archives/{archive}'.sub('{' + 'system_name' + '}', system_name.to_s).sub('{' + 'archive' + '}', archive.to_s)
	...
	:return_type => {
	  '200' => 'Appliance::ApplianceRecoveryBackupSystemNameArchiveResp',
    ...
    # @return [ApplianceRecoveryBackupSystemNameArchiveListResp|VapiStdErrorsErrorError|VapiStdErrorsNotFoundError|]
    ...
    # @option opts [String] :filter_spec_comment_substring Backup comment must contain this {@term string} to match the filter.
    # @option opts [Integer] :filter_spec_max_results Limit result to a max count of most recent backups.
    # @return [Array<(ApplianceRecoveryBackupSystemNameArchiveListResp|VapiStdErrorsErrorError|VapiStdErrorsNotFoundError|, Fixnum, Hash)>]  data, response status code and response headers
    def list_with_http_info(system_name, loc_spec_location, opts = {})
      ...
      # resource path
      local_var_path = '/rest/appliance/recovery/backup/system-name/{system_name}/archives'.sub('{' + 'system_name' + '}', system_name.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :auth_names => auth_names,
	:return_type => {
	  '200' => 'Appliance::ApplianceRecoveryBackupSystemNameArchiveListResp',
	  '400' => 'Appliance::VapiStdErrorsErrorError',
	  '404' => 'Appliance::VapiStdErrorsNotFoundError',
	})
      if @api_client.config.debugging
