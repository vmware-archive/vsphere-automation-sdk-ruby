...
module VSphereAutomation
  ...
  class DeploymentInstallPscReplicatedApi
    ...
    def check(vcenter_deployment_install_psc_replicated_check, opts = {})
      data, _status_code, _headers = check_with_http_info(vcenter_deployment_install_psc_replicated_check, opts)
      data
    end
    ...
    def check_with_http_info(vcenter_deployment_install_psc_replicated_check, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DeploymentInstallPscReplicatedApi.check ...'
      end
      # verify the required parameter 'vcenter_deployment_install_psc_replicated_check' is set
      if @api_client.config.client_side_validation && vcenter_deployment_install_psc_replicated_check.nil?
        fail ArgumentError, "Missing the required parameter 'vcenter_deployment_install_psc_replicated_check' when calling DeploymentInstallPscReplicatedApi.check"
      end
      # resource path
      ...
      # http body (model)
      post_body = @api_client.object_to_http_body(vcenter_deployment_install_psc_replicated_check)
      auth_names = []
	...
	:return_type => {
	  '200' => 'VCenter::VcenterDeploymentInstallPscReplicatedCheckResult',
