...
module VSphereAutomation
  ...
  class NsxClustersApi
    ...
    # @return [VcenterNsxClustersInfo|VapiStdErrorsNotFound|VapiStdErrorsError|]
    ...
    # @param cluster Identifier for the vSphere cluster on which NSX networking is enabled. The parameter must be an identifier for the resource type: ClusterComputeResource.
    # @param [Hash] opts the optional parameters
    # @return [Array<(VcenterNsxClustersInfo|VapiStdErrorsNotFound|VapiStdErrorsError|, Fixnum, Hash)>]  data, response status code and response headers
    def get_with_http_info(cluster, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: NsxClustersApi.get ...'
      end
      # verify the required parameter 'cluster' is set
      if @api_client.config.client_side_validation && cluster.nil?
        fail ArgumentError, "Missing the required parameter 'cluster' when calling NsxClustersApi.get"
      end
      # resource path
      local_var_path = '/api/vcenter/nsx/clusters/{cluster}'.sub('{' + 'cluster' + '}', cluster.to_s)
	...
	:return_type => {
	  '200' => 'VCenter::VcenterNsxClustersInfo',
	  '404' => 'VCenter::VapiStdErrorsNotFound',
	  '500' => 'VCenter::VapiStdErrorsError',
	})
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: NsxClustersApi#get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
