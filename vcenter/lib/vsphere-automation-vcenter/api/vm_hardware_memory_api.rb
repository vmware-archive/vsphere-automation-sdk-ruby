...
module VSphereAutomation
  ...
  class VmHardwareMemoryApi
    ...
    # @return [VcenterVmHardwareMemoryResult|VapiStdErrorsResourceInaccessibleError|VapiStdErrorsUnauthenticatedError|VapiStdErrorsUnauthorizedError|VapiStdErrorsNotFoundError|VapiStdErrorsServiceUnavailableError|]
    def get(vm, opts = {})
    ...
    # @param vm Virtual machine identifier. The parameter must be an identifier for the resource type: VirtualMachine.
    # @param [Hash] opts the optional parameters
    # @return [Array<(VcenterVmHardwareMemoryResult|VapiStdErrorsResourceInaccessibleError|VapiStdErrorsUnauthenticatedError|VapiStdErrorsUnauthorizedError|VapiStdErrorsNotFoundError|VapiStdErrorsServiceUnavailableError|, Fixnum, Hash)>]  data, response status code and response headers
	...
	:return_type => {
	  '200' => 'VCenter::VcenterVmHardwareMemoryResult',
    ...
    def update(vm, vcenter_vm_hardware_memory_update, opts = {})
      update_with_http_info(vm, vcenter_vm_hardware_memory_update, opts)
      nil
    end
    ...
    def update_with_http_info(vm, vcenter_vm_hardware_memory_update, opts = {})
      if @api_client.config.debugging
      ...
      # verify the required parameter 'vcenter_vm_hardware_memory_update' is set
      if @api_client.config.client_side_validation && vcenter_vm_hardware_memory_update.nil?
        fail ArgumentError, "Missing the required parameter 'vcenter_vm_hardware_memory_update' when calling VmHardwareMemoryApi.update"
      end
      # resource path
      local_var_path = '/vcenter/vm/{vm}/hardware/memory'.sub('{' + 'vm' + '}', vm.to_s)
      ...
      # http body (model)
      post_body = @api_client.object_to_http_body(vcenter_vm_hardware_memory_update)
      auth_names = []
