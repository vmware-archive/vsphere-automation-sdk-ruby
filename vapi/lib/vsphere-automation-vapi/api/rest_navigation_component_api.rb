...
module VSphereAutomation
  ...
  class RestNavigationComponentApi
    ...
    # @return [VapiRestNavigationComponentListResp|]
    def list(opts = {})
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :base_url Base URL. The {param.name baseUrl} parameter is used by the service to produce absolute URLs. If unset then the service will produce relative URLs.
    # @return [Array<(VapiRestNavigationComponentListResp|, Fixnum, Hash)>]  data, response status code and response headers
    def list_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: RestNavigationComponentApi.list ...'
      end
      # resource path
      local_var_path = '/rest/com/vmware/vapi/rest/navigation/component'
	...
	:return_type => {
	  '200' => 'VAPI::VapiRestNavigationComponentListResp',
	})
      if @api_client.config.debugging
