...
All samples support vCenter 6.8. Tagging samples also support vCenter 6.0.
...
The Gem specification file (`sdk/vsphere-automation-sdk.gemspec`) lists the
...
### Installing the SDK
You can install the SDK, by going into each subfolder with a `.gemspec` file and running:

```cmd
gem build vsphere-automation-*.gemspec
gem install vsphere-automation-*.gem
```

### Uninstalling the SDK
Uninstall the SDK by running (on Linux or macOS)

```
gem list --local --no-versions | grep vsphere-automation | xargs gem uninstall --force
```

On Windows, you'll need to manually `gem uninstall` each Gem that begins with `vsphere-automation-`
...
## API Documentation
The documentation is in the `docs` folder in each section (`cis`, `content`, etc.). Open the Markdown file for the API or Model object you'd like to look at.
