...
module VSphereAutomation
  module ESX
  class SettingsClustersSoftwareAddOnApi
    ...
    def get_with_http_info(cluster, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SettingsClustersSoftwareAddOnApi.get ...'
      end
      # verify the required parameter 'cluster' is set
      if @api_client.config.client_side_validation && cluster.nil?
        fail ArgumentError, "Missing the required parameter 'cluster' when calling SettingsClustersSoftwareAddOnApi.get"
      end
      # resource path
      local_var_path = '/api/esx/settings/clusters/{cluster}/software/add-on'.sub('{' + 'cluster' + '}', cluster.to_s)
	...
	:return_type => {
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SettingsClustersSoftwareAddOnApi#get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
