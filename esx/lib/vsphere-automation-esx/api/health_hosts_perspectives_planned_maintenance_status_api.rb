...
module VSphereAutomation
  module ESX
  class HealthHostsPerspectivesPlannedMaintenanceStatusApi
    ...
    def check_with_http_info(host, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: HealthHostsPerspectivesPlannedMaintenanceStatusApi.check ...'
      end
      # verify the required parameter 'host' is set
      if @api_client.config.client_side_validation && host.nil?
        fail ArgumentError, "Missing the required parameter 'host' when calling HealthHostsPerspectivesPlannedMaintenanceStatusApi.check"
      end
      # resource path
      local_var_path = '/api/esx/health/hosts/{host}/perspectives/planned-maintenance/status?action=check'.sub('{' + 'host' + '}', host.to_s)
	...
	:return_type => {
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: HealthHostsPerspectivesPlannedMaintenanceStatusApi#check\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
