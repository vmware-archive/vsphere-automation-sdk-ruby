...
module VSphereAutomation
  module ESX
  class SettingsHostsSoftwareBaseImageApi
    ...
    def get_with_http_info(host, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SettingsHostsSoftwareBaseImageApi.get ...'
      end
      # verify the required parameter 'host' is set
      if @api_client.config.client_side_validation && host.nil?
        fail ArgumentError, "Missing the required parameter 'host' when calling SettingsHostsSoftwareBaseImageApi.get"
      end
      # resource path
      local_var_path = '/api/esx/settings/hosts/{host}/software/base-image'.sub('{' + 'host' + '}', host.to_s)
	...
	:return_type => {
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SettingsHostsSoftwareBaseImageApi#get\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
