...
module VSphereAutomation
  module ESX
  class SettingsClustersSoftwareSolutionsNsxNsxInternalApi
    ...
    def apply_task_with_http_info(cluster, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SettingsClustersSoftwareSolutionsNsxNsxInternalApi.apply_task ...'
      end
      # verify the required parameter 'cluster' is set
      if @api_client.config.client_side_validation && cluster.nil?
        fail ArgumentError, "Missing the required parameter 'cluster' when calling SettingsClustersSoftwareSolutionsNsxNsxInternalApi.apply_task"
      end
      # resource path
      local_var_path = '/api/esx/settings/clusters/{cluster}/software/solutions/nsx?action=apply&vmw-task=true'.sub('{' + 'cluster' + '}', cluster.to_s)
	...
	:return_type => {
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SettingsClustersSoftwareSolutionsNsxNsxInternalApi#apply_task\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
