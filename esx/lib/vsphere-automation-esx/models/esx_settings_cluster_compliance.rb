# Copyright (c) 2018-2019 VMware, Inc.  All Rights Reserved.
# SPDX-License-Identifier: MIT

# DO NOT MODIFY. THIS CODE IS GENERATED. CHANGES WILL BE OVERWRITTEN.

# esx - No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)


require 'date'

module VSphereAutomation
  module ESX
    class EsxSettingsClusterCompliance
    # Identifier of the commit on which the scan is run to generate this result.
    attr_accessor :commit

    # Identifiers of compliant hosts.
    attr_accessor :compliant_hosts

    # Auxillary information about the hosts. This gives some additional information about the hosts referenced in this result.
    attr_accessor :host_info

    # Mapping from host identifier to the compliance information for the host.
    attr_accessor :hosts

    attr_accessor :impact

    # Identifiers of incompatible hosts.
    attr_accessor :incompatible_hosts

    # Identifiers of non-compliant hosts.
    attr_accessor :non_compliant_hosts

    attr_accessor :notifications

    # Scan completion time.
    attr_accessor :scan_time

    attr_accessor :status

    # Identifiers of unavailable hosts. There will not be compliance details for these hosts in {@link #hosts}.
    attr_accessor :unavailable_hosts

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'commit' => :'commit',
        :'compliant_hosts' => :'compliant_hosts',
        :'host_info' => :'host_info',
        :'hosts' => :'hosts',
        :'impact' => :'impact',
        :'incompatible_hosts' => :'incompatible_hosts',
        :'non_compliant_hosts' => :'non_compliant_hosts',
        :'notifications' => :'notifications',
        :'scan_time' => :'scan_time',
        :'status' => :'status',
        :'unavailable_hosts' => :'unavailable_hosts'
      }
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'commit' => :'String',
        :'compliant_hosts' => :'Array<String>',
        :'host_info' => :'Hash<String, EsxSettingsHostInfo>',
        :'hosts' => :'Hash<String, EsxSettingsHostCompliance>',
        :'impact' => :'EsxSettingsComplianceImpact',
        :'incompatible_hosts' => :'Array<String>',
        :'non_compliant_hosts' => :'Array<String>',
        :'notifications' => :'EsxSettingsNotifications',
        :'scan_time' => :'DateTime',
        :'status' => :'EsxSettingsComplianceStatus',
        :'unavailable_hosts' => :'Array<String>'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      # convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h| h[k.to_sym] = v }

      if attributes.has_key?(:'commit')
        self.commit = attributes[:'commit']
      end

      if attributes.has_key?(:'compliant_hosts')
        if (value = attributes[:'compliant_hosts']).is_a?(Array)
          self.compliant_hosts = value
        end
      end

      if attributes.has_key?(:'host_info')
        if (value = attributes[:'host_info']).is_a?(Hash)
          self.host_info = value
        end
      end

      if attributes.has_key?(:'hosts')
        if (value = attributes[:'hosts']).is_a?(Hash)
          self.hosts = value
        end
      end

      if attributes.has_key?(:'impact')
        self.impact = attributes[:'impact']
      end

      if attributes.has_key?(:'incompatible_hosts')
        if (value = attributes[:'incompatible_hosts']).is_a?(Array)
          self.incompatible_hosts = value
        end
      end

      if attributes.has_key?(:'non_compliant_hosts')
        if (value = attributes[:'non_compliant_hosts']).is_a?(Array)
          self.non_compliant_hosts = value
        end
      end

      if attributes.has_key?(:'notifications')
        self.notifications = attributes[:'notifications']
      end

      if attributes.has_key?(:'scan_time')
        self.scan_time = attributes[:'scan_time']
      end

      if attributes.has_key?(:'status')
        self.status = attributes[:'status']
      end

      if attributes.has_key?(:'unavailable_hosts')
        if (value = attributes[:'unavailable_hosts']).is_a?(Array)
          self.unavailable_hosts = value
        end
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      if @compliant_hosts.nil?
        invalid_properties.push('invalid value for "compliant_hosts", compliant_hosts cannot be nil.')
      end

      if @host_info.nil?
        invalid_properties.push('invalid value for "host_info", host_info cannot be nil.')
      end

      if @hosts.nil?
        invalid_properties.push('invalid value for "hosts", hosts cannot be nil.')
      end

      if @impact.nil?
        invalid_properties.push('invalid value for "impact", impact cannot be nil.')
      end

      if @incompatible_hosts.nil?
        invalid_properties.push('invalid value for "incompatible_hosts", incompatible_hosts cannot be nil.')
      end

      if @non_compliant_hosts.nil?
        invalid_properties.push('invalid value for "non_compliant_hosts", non_compliant_hosts cannot be nil.')
      end

      if @notifications.nil?
        invalid_properties.push('invalid value for "notifications", notifications cannot be nil.')
      end

      if @scan_time.nil?
        invalid_properties.push('invalid value for "scan_time", scan_time cannot be nil.')
      end

      if @status.nil?
        invalid_properties.push('invalid value for "status", status cannot be nil.')
      end

      if @unavailable_hosts.nil?
        invalid_properties.push('invalid value for "unavailable_hosts", unavailable_hosts cannot be nil.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      return false if @compliant_hosts.nil?
      return false if @host_info.nil?
      return false if @hosts.nil?
      return false if @impact.nil?
      return false if @incompatible_hosts.nil?
      return false if @non_compliant_hosts.nil?
      return false if @notifications.nil?
      return false if @scan_time.nil?
      return false if @status.nil?
      return false if @unavailable_hosts.nil?
      true
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          commit == o.commit &&
          compliant_hosts == o.compliant_hosts &&
          host_info == o.host_info &&
          hosts == o.hosts &&
          impact == o.impact &&
          incompatible_hosts == o.incompatible_hosts &&
          non_compliant_hosts == o.non_compliant_hosts &&
          notifications == o.notifications &&
          scan_time == o.scan_time &&
          status == o.status &&
          unavailable_hosts == o.unavailable_hosts
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Fixnum] Hash code
    def hash
      [commit, compliant_hosts, host_info, hosts, impact, incompatible_hosts, non_compliant_hosts, notifications, scan_time, status, unavailable_hosts].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.openapi_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :BOOLEAN, :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        temp_model = VSphereAutomation::ESX.const_get(type).new
        temp_model.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end

  end
  end
end
